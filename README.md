...
The finite verification used in Section 5 is available both as an unpacked directory and as a packaged archive:
...
GitHub Actions runs the exact certificate on pushes and pull requests, regenerates the fixed Atlas snapshots with NetworkX 3.6.1 when needed, compiles the manuscript, and packages the archive.
...
Package provenance and reproducibility information are recorded in [`FIRST_PUBLICATION_RECORD.md`](FIRST_PUBLICATION_RECORD.md). This record is not a substitute for peer review or independent endorsement.
