# Repository Publication and Integrity Record

**Repository:** \`FDmd233/strengthened-szeged-wiener-conjecture\`

**First complete public artifact commit:** \`f4dd375fbedc5153cc7aa13c8102f1c0f4eb3056\`

**Artifact build time (UTC):** \`2026-09-08T17:32:11Z\`

**Verification workflow run:** \`34257517977\`

The commit above is the first commit in this repository containing the complete manuscript source, a compiled PDF, the unpacked exact finite certificate, the fixed Graph Atlas input snapshots, and a downloadable certificate archive.

GitHub Actions generated the public artifacts from the manuscript and certificate sources. In the same run, NetworkX 3.6.1 regenerated the fixed Atlas snapshots, their SHA256 digests were checked, the C++17 certificate was compiled and executed, and the manuscript was compiled with XeLaTeX. The workflow completed successfully.

The exact certificate reproduced the critical finite outputs, including

\`\`\`text
F8 97
H9 681
G10 300311 min 20 bad 0
L5=10 pieces 12
L6=8 pieces 209
L7=11 pieces 3258
L8=14 pieces 48865
order8 nonspecial eta<=14 labelled_ext 65 L9_from_low=17
n10 105 min 20 bad 0
n11 160 min 23 bad 0
certificate verification: PASS
\`\`\`

## Status note

This record documents public availability, reproducibility, and file integrity. It does **not** constitute peer review or independent mathematical endorsement. The manuscript is presented as a claimed proof of Conjecture 5 of Bonamy, Knor, Luzar, Pinlou and Skrekovski (2017); at the time of this record, MathDB still lists the problem as open and independent verification has been requested.
